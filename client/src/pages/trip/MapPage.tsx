...
import { Plus, Trash2, Loader2, MapPin, Hotel, Utensils, Car, MoreHorizontal } from "lucide-react";
...
export default function MapPage({ tripId }: { tripId: number }) {
  ...
  const [form, setForm] = useState({ title: "", description: "", lat: "", lng: "", type: "attraction" });
  const mapRef = useRef<google.maps.Map | null>(null);
  const markersRef = useRef<google.maps.Marker[]>([]);
  const infoWindowRef = useRef<google.maps.InfoWindow | null>(null);

  const addPin = trpc.map.addPin.useMutation({
    onSuccess: () => { refetch(); setShowAdd(false); setForm({ title: "", description: "", lat: "", lng: "", type: "attraction" }); toast.success("地點已新增"); },
  ...
  // Use effect equivalent via key change
  const mapKey = pins?.map(p => p.id).join(",") ?? "";

  const handleAdd = () => {
    if (!form.title || !form.lat || !form.lng) { toast.error("請填寫名稱並在地圖上選擇位置"); return; }
    addPin.mutate({ tripId, title: form.title, description: form.description, lat: form.lat, lng: form.lng, type: form.type as "attraction" | "hotel" | "restaurant" | "transport" | "other" });
  };
    ...
    <div className="flex flex-col h-full" style={{height:"calc(100vh - 120px)"}}>
          ...
          {!pins || pins.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {pins.map(pin => {
                ...
                return (
                  <div key={pin.id} className={`p-3 flex items-start gap-3 group cursor-pointer hover:bg-accent/50 transition-colors ${selectedPin?.id === pin.id ? "bg-accent" : ""}`}
                    onClick={() => {
                      setSelectedPin(pin);
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>新增地點標記</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            <Button className="w-full" onClick={handleAdd} disabled={addPin.isPending}>
